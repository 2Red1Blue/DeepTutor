...
Every native invocation also acquires a non-blocking process lock below the shared DeepTutor runtime home. The process retains that lock for its lifetime. This closes launch paths that lie about `backend_workers`, including a direct `uvicorn ... --workers 2` with the stored setting left at `1`: only one local worker can resolve a deployment backend or Partner, and every competing worker fails closed before backend execution. The lock is local-host protection, not a distributed lease; separate hosts with separate runtime filesystems remain unsupported. Redis turn coordination does not satisfy this separate lease requirement.
