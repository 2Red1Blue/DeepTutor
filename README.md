...
Connected Agents are native DeepTutor execution, not managed Personal Runtime or Agent Fabric work. Local CLIs require an explicit directory inside the selected workspace; deployment-owned CLI and remote-gateway credentials are admin-only unless the backend is explicitly assigned to an account. Until their backend-session registry gains a distributed lease, `backend_workers > 1` requires every Connected Agent backend to be disabled, and a process lock makes competing local worker invocation fail closed even when an external launcher lies about that setting. Persistent connection incarnations also prevent a late consult from reviving a deleted connection or leaking its session into a recreated backend. See [the runtime contract](docs-for-user/connected-agents-runtime-contract.md).
